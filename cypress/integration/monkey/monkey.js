...
function getEvtType(i) {
    ...
    else if (i === 2) return "Random click"
...
var pending_events = [, , , , ,]
function login() {
    if (userName === "No tiene asignado") {
        throw 'No tiene correo asignado en monkey-config.json >> email';
    }
    cy.get('form').within(() => {
...
describe(`${appName} under monkeys - Posts`, function () {
    //Listener
...
xdescribe(`${appName} under monkeys - Members`, function () {
    //Listener
    cy.on('uncaught:exception', (err) => {
        cy.task('genericLog', { 'message': `An exception occurred: ${err}` });
        cy.task('genericReport', { 'html': `<p><strong>Uncaught exception: </strong>${err}</p>` });
    ...
    it(`visits ${appName} and survives monkeys`, function () {
        ...
        if (pcg === 100) {
            ...
            login();
            cy.wait(1000)
            cy.get("li.relative > a[href='#/members/']").click()
            cy.wait(1000)
...
xdescribe(`${appName} under monkeys - Pages`, function () {
    //Listener
    cy.on('uncaught:exception', (err) => {
        cy.task('genericLog', { 'message': `An exception occurred: ${err}` });
        cy.task('genericReport', { 'html': `<p><strong>Uncaught exception: </strong>${err}</p>` });
